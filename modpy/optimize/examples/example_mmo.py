...
from modpy.optimize import mmo, cma_es, nlprog
...
if __name__ == '__main__':
    ...
    #def core_wrap(obj_, x0, bounds=None, constraints=(), sigma0=0.3):
    #    return cma_es(obj_, x0, bounds=bounds, constraints=constraints, sigma0=sigma0)

    def core_wrap(obj_, x0, bounds=None, constraints=(), sigma0=1., C0=None):
        return cma_es(obj_, x0, bounds=bounds, constraints=constraints, sigma0=sigma0, C0=C0)#nlprog(obj_, x0, bounds=bounds, constraints=constraints)

    # wrap population generation function
    def pop_wrap(N):
        return np.random.uniform(-5., 5., (N, 2))

    E = mmo(test_function_2d, core_wrap, pop_wrap, 2, peaks=4, V=100.).results
    ...
    fig.savefig(OPTIMIZE_PATH + 'mmo_hill_valley.png')
